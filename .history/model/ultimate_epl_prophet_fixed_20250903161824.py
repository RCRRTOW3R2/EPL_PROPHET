    ...
    for home, away in test_matches:
        ...
        if result['top_factors']:
            ...
            for factor in result['top_factors'][:3]:
                ...
                impact_val = float(impact) if np.isscalar(impact) else float(impact[0])
                direction = "→ Home" if impact_val > 0 else "→ Away" if impact_val < 0 else "→ Neutral"
                print(f"      • {factor['feature']}: {impact_val:+.3f} {direction}")
